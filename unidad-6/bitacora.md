...
2. Codigo:
```js
// sketch.js - flowfield + flock dentro de ondas + audio-reactive (guitarra controla velocidad)
// - Coloca el mp3 en assets/Pokemon_Atrapalos_Ya_Opening_1.mp3
// - Click: crea onda y alterna play/pause
// - 1: mostrar/ocultar flowfield
// - 2: modo círculos
// - 3: modo cuadrados
// - C: despejar (reubicar) partículas
...
function draw() {
  ...
  let guitarEnergy = fft.getEnergy(600, 1000);
  let guitarSpeed = map(guitarEnergy, 0, 255, 1.5, 6);


  for (let p of particles) {
    p.maxspeed = guitarSpeed;
  }
...
function mousePressed() {
  ...
  if (song) {
    if (!song.isPlaying()) {
      if (getAudioContext && getAudioContext().state !== 'running') userStartAudio();
      song.loop();
    } else {
...
class Particle {
  constructor() {
    ...
    this.color = random(1) < 0.5 ? color(255, 255, 255) : color(255, 0, 0);
  }
  ...
  show(spectrum) {
    ...
    let idx = int(map(this.pos.x, 0, width, 0, max(1, spectrum.length - 1)));
    idx = constrain(idx, 0, max(0, spectrum.length - 1));
    let freq = spectrum[idx] || 0;
    let r = map(freq, 0, 255, 6, 24);
...
<img width="960" height="696" alt="image" src="https://github.com/user-attachments/assets/cff47c7d-33d3-40f9-b935-3f0a30640772" />
